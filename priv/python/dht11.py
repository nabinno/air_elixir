...
def register(pid, pin):
    global MESSAGE_HANDLER_PID
    global DHT11_INSTANCE
    MESSAGE_HANDLER_PID = pid
    DHT11_INSTANCE = DHT11(pin)
    return (Atom('ok'), pid)


def read():
    if MESSAGE_HANDLER_PID is None or DHT11_INSTANCE is None:
        return (Atom('error'), (0, 0))

    result = DHT11_INSTANCE.read()
    if result.is_valid():
        return (Atom('ok'), (result.temperature, result.humidity))
    else:
        return (Atom('error'), (0, 0))
